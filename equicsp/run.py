...
from equicsp.common.utils import log_hyperparameters, PROJECT_ROOT
...
def print_config(cfg):
    ...
    console.print(
        Align.center(
            Panel.fit(
                syntax,
                title=Text("⚙ Training Configuration ⚙", style="bold yellow"),
...
class RichEpochSummary(Callback):
...
def build_callbacks(cfg: DictConfig) -> List[Callback]:
    ...
    if "progress_bar_refresh_rate" in cfg.logging:
        hydra.utils.log.info("Adding callback <RichProgressBar>")
        callbacks.append(RichProgressBar())
        hydra.utils.log.info("Adding custom callback <RichEpochSummary>")
        callbacks.append(RichEpochSummary())
...
def run(cfg: DictConfig) -> None:
    ...
    if datamodule.scaler is not None:
        ...
        model.coord_norm_factor = datamodule.coord_norm_factor

    torch.save(datamodule.lattice_scaler, hydra_dir / "lattice_scaler.pt")
    torch.save(datamodule.scaler, hydra_dir / "prop_scaler.pt")
    torch.save(datamodule.coord_norm_factor, hydra_dir / "coord_norm_factor.pt")

    ### NEW: print model summary for multi-poly
    ...
    if hasattr(model, 'decoder'):
        for name, module in model.decoder.named_children():
            ...
            if params > 0:
                print(f"  decoder.{name}: {params:,} params")
    print()
    ### END NEW
