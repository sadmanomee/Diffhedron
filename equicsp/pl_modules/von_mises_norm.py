...
def get_trans_x(n, mu=0.0, sigma=0.1):
    ...
    invarance_reference = calc_grouped_angles_mean_in_radians(
        rand_x.view(-1, 1),
        torch.tensor([i for i in range(sample_size) for _ in range(n)]),
    )
    invarance_reference = invarance_reference.repeat_interleave(n, dim=0)
    invariance_noise = ((rand_x.view(-1, 1) - invarance_reference) % 1.0).view(-1)
    ...
    for _ in trange(sn):
        ...
        for n in range(1, num_atoms + 1):
            ...
            mean_angle = torch.where(
                mean_angle >= 0, mean_angle, mean_angle + 2 * math.pi
            )
...
if __name__ == "__main__":
    ...
    for n in range(2, max_atoms + 1):
        ...
        for i in trange(len(sigmas)):
            ...
            bounds = [(1e-6, 10000)]
            result = minimize(
                neg_log_likelihood, initial_guess, bounds=bounds, args=(data,)
            )
            kappa = result.x[0]
    ...
    sigmas = torch.FloatTensor(
        np.exp(np.linspace(np.log(0.005), np.log(0.5), 1000))
    ).to("cuda")
