...
class CSPLayer(nn.Module):
    ...
    def edge_model(
        ...
        lattice_ips_flatten = lattice_ips
...
class CSPLayerSelfAttn(nn.Module):
    """Message passing layer for cspnet."""
    ...
    def __init__(
        ...
        super(CSPLayerSelfAttn, self).__init__()
        ...
        self.k_transform = nn.Sequential(
            nn.Linear(hidden_dim + 9 + self.dis_dim, hidden_dim * 2),
            act_fn,
            nn.Linear(hidden_dim * 2, hidden_dim),
            act_fn,
        )
    ...
    def edge_model(
        ...
        lattice_ips_flatten_edges = lattice_ips_flatten[edge2graph]
        edges_input = torch.cat([hj, lattice_ips_flatten_edges, frac_diff], dim=1)
        q = self.q_transform(node_features)[edge_index[0]]
        k = self.k_transform(edges_input)
        v = self.v_transform(edges_input)
        q, k, v = map(
            lambda t: rearrange(t, "e (h d) -> e h d", h=self.head), (q, k, v)
        )
        scores = (q * k).sum(dim=-1) / math.sqrt(self.head_dim)
        attn = scatter_softmax(scores, edge_index[0], dim=0).unsqueeze(-1)
        agg = scatter(
            attn * v,
            edge_index[0],
            dim=0,
        ...
        agg = rearrange(agg, "e h d -> e (h d)", h=self.head)
        return agg
...
class CSPNet(nn.Module):
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        # print(f't_per_atom shape: {t_per_atom.shape}')
        ...
        # node_features = (node_features + node_features.mean(dim=1, keepdim=True)) * 0.5 # context broadcasting
