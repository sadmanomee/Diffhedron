...
recommand_step_lr = {
    "csp": {"perov_5": 5e-7, "carbon_24": 5e-6, "mp_20": 5e-6, "mpts_52": 1e-5},
    "csp_multi": {"perov_5": 5e-7, "carbon_24": 5e-7, "mp_20": 5e-6, "mpts_52": 1e-5},
...
def get_model_path(eval_model_name):
    ...
    model_path = Path(equicsp.__file__).parent / "prop_models" / eval_model_name
    return model_path
...
def load_model(
    model_path, load_data=False, testing=True, last_ckpt=True, load_scaler=False
):
    with initialize_config_dir(str(model_path)):
        cfg = compose(config_name="hparams")
        ...
        for i in range(num_dataset):
            test_dataset = [
                hydra.utils.instantiate(
                    dataset_cfg,
                    _target_="equicsp.pl_data.dataset.DisCrystDataset",
                    rank=i,
                    num_dis=num_dataset,
                )
